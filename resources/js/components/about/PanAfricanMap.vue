<template>
  <section class="py-5 bg-white overflow-hidden">
    <div class="max-w-6xl mx-auto px-4">
      ...
      <div class="relative bg-slate-50 rounded-[40px] shadow-inner p-4 md:p-6 border border-gray-100">
        <svg viewBox="0 0 1000 1001" xmlns="http://www.w3.org/2000/svg"
          class="w-full h-auto max-h-[85vh] filter drop-shadow-2xl" id="africa-map">

          <path v-for="country in countriesData" :key="country.id" :id="country.id" :d="country.d"
            :class="{ 'is-cofina': cofinaConfig[country.id] }" @mouseover="hoveredCountry = country.id"
            @mouseleave="hoveredCountry = null" />

          <g v-for="(config, id) in cofinaConfig" :key="'pulse-' + id" class="pointer-events-none">
            <circle :cx="config.x" :cy="config.y" r="12" :fill="id === 'CI' ? '#FF4D4D' : '#D10000'"
              class="animate-pulse-slow opacity-40" />
            <circle :cx="config.x" :cy="config.y" r="5" :fill="id === 'CI' ? '#FF4D4D' : '#D10000'" />
          </g>

          <foreignObject v-if="hoveredCountry && cofinaConfig[hoveredCountry]" :x="cofinaConfig[hoveredCountry].x - 60"
            :y="cofinaConfig[hoveredCountry].y - 100" width="120" height="90"
            class="pointer-events-none transition-all duration-300">
            <div class="flex flex-col items-center animate-float">
              <div class="bg-white p-1.5 rounded-lg shadow-xl border border-gray-100 flex flex-col items-center">
                <img :src="`https://flagcdn.com/w80/${hoveredCountry.toLowerCase()}.png`"
                  class="w-10 h-6 object-cover rounded shadow-sm" />
                <span class="text-[11px] font-black text-gray-800 uppercase mt-1 tracking-tighter">
                  {{ cofinaConfig[hoveredCountry].name }}
                </span>
              </div>
              <div class="w-3 h-3 bg-white rotate-45 -mt-1.5 border-r border-b border-gray-100 shadow-sm"></div>
            </div>
          </foreignObject>
        </svg>

        <div class="absolute bottom-8 right-8 space-y-4">
          <div class="bg-white/90 backdrop-blur-md p-4 rounded-2xl border border-gray-100 shadow-lg hidden md:block">
            <div class="flex items-center gap-3 mb-2">
              <img src="https://flagcdn.com/w40/fr.png" class="w-6 h-4 shadow-sm rounded-sm" />
              <span class="text-sm font-bold text-gray-800">Cofina France</span>
            </div>
            <p class="text-[11px] text-gray-500 leading-tight">Bureau de représentation<br>à Paris (75008)</p>
          </div>

          <div class="bg-white/90 backdrop-blur-md p-4 rounded-2xl border border-gray-100 shadow-lg">
            <div class="flex items-center gap-2 mb-2">
              <div class="w-3 h-3 rounded-full bg-[#FF4D4D]"></div>
              <span class="text-xs font-bold text-gray-700">Siège (Côte d'Ivoire)</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-3 h-3 rounded-full bg-primary"></div>
              <span class="text-xs font-bold text-gray-700">Filiales Pays</span>
            </div>
          </div>
        </div>
      </div>
...
// Configuration des pays COFINA (IDs basés sur ton SVG)
// Les coordonnées X et Y sont ajustées pour tomber au centre des pays sur ton SVG 1000x1001
const cofinaConfig = {
  'SN': { name: 'Sénégal', x: 80, y: 310 },
  'ML': { name: 'Mali', x: 230, y: 260 },
  'GN': { name: 'Guinée', x: 135, y: 385 },
...
const countriesData = [
  ...
  { id: 'ZW', d: 'm 669.1,825.1 13.4,-15.7 3.6,-9.9 1.9,-1.3 1.7,-8.1 -1.7,-4.1 1,-10.4 2.8,-9.6 0.8,-17.5 -6.1,-4.4 -5.7,-1 -2.5,-3.4 -5.5,-3 -10,0.3 -0.6,-5.1 -9.3,1.9 -7,5.5 -1.7,4.8 -4.4,1 -10.9,11.4 -7,9 -4.1,0.3 -3.8,-1.6 -13.4,-1.5 4.2,11.1 2.3,2.4 3.5,8 13,15.2 5.1,1.5 -0.2,4.9 3.1,8.8 9.2,2.1 7.3,6.2 4.9,0.2 5.7,2.3 4.1,-1.6 6.3,1.3 z' },
  { id: 'SO', d: 'm 832.6,372.8 -5.7,-5.8 -2.5,-5.7 -4.1,-2.6 -4.2,7.4 -2.5,4.9 4.8,7.7 4.6,6.6 4.8,5 40.3,16.4 10.3,-0.1 -33.5,41.5 -15.9,0.6 -10.7,9.8 -7.8,0.2 -3.3,4.4 -10.5,15.7 0.4,50.4 7.2,11.4 2.7,-3.3 2.9,-7.3 13.4,-16.7 11.4,-10.6 18.1,-13.7 12.1,-11.2 14.1,-18.9 10.1,-15.5 10,-20.2 7,-17.7 5.4,-15.5 2.9,-14.9 2.4,-5 -0.4,-7.3 0.8,-8 -0.5,-3.9 -4.6,0.1 -5.5,4.7 -6.4,1.4 -5.5,2.1 -3.9,0.2 0,0 -6.9,0.5 -4.2,2.6 -6,0.9 -10.5,4.3 -13.2,1.6 -11.3,3.5 -6.1,0 z' }
];
</script>
...
/* Style par défaut des pays COFINA */
.is-cofina {
  fill: #D10000;
  cursor: pointer;
}
...
/* Style spécifique Côte d'Ivoire (Rouge plus clair) */
#CI.is-cofina {
  fill: #FF4D4D;
}
...
/* Pays non-COFINA au survol */
path:not(.is-cofina):hover {
  fill: #cbd5e1;
}
...
#africa-map {
  /* Améliore le rendu des bords du SVG sur Chrome/Safari */
