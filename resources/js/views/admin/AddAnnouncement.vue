  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-4">
    <div class="max-w-5xl mx-auto">
      ...
      <div class="mb-8 flex items-center justify-between">
        <div>
          <h1 class="text-4xl font-bold text-gray-900 mb-2">Gestion des annonces</h1>
          <p class="text-gray-600">Gérez, créez et modifiez vos annonces</p>
        </div>
        <button
          @click="scrollToForm"
      ...
      <!-- Section Tableau des annonces -->
      <div class="bg-white rounded-2xl shadow-xl p-8 mb-8">
        <h2 class="text-xl font-bold text-gray-900 mb-6">Liste des annonces</h2>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-if="filteredAnnouncements.length === 0">
                <td colspan="3" class="px-6 py-12 text-center text-gray-500">
                  ...
                  <p class="text-sm">Commencez par ajouter votre première annonce</p>
                </td>
              </tr>
              <!-- Lignes du tableau -->
              <tr v-for="item in filteredAnnouncements" :key="item.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4">
      ...
      <div ref="formSection" class="bg-white rounded-2xl shadow-xl p-8 space-y-8">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-gray-600 mt-1">
              {{ isEditing ? 'Modifiez les informations de l\'annonce' : 'Renseignez les informations de la nouvelle annonce' }}
            </p>
          </div>
          <button
            v-if="isEditing"
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ex: Nouvelle offre promotionnelle..."
          />
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t">
          ...
          >
            ...
            {{ isEditing ? 'Sauvegarder les modifications' : 'Ajouter l\'annonce' }}
          </button>
        </div>
      </div>
    </div>
...
// Computed pour filtrer les annonces
const filteredAnnouncements = computed(() => {
  return announcements.value.filter(a => {
    const matchesSearch = (a.content || '').toLowerCase().includes(searchQuery.value.toLowerCase())
    const matchesStatus =
...
const fetchAnnouncements = async () => {
  try {
    const response = await api.get('/announcements?with_user=true')
...
const saveAnnouncement = async () => {
  if (!announcement.value.content || announcement.value.is_published === '') {
  ...
  const payload = {
    ...
    is_published: announcement.value.is_published,
  }
